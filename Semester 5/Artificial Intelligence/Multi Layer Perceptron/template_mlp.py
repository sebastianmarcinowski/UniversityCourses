...
class MLPApproximator(BaseEstimator, RegressorMixin):
    ...
    @staticmethod
    def relu_d(phi_S):
        # return np.where(phi_S < 0, 0, np.where(phi_S == 0, np.nan, 1))
        return np.where(phi_S > 0, 1, 0)
    ...
    def squared_loss(y_MLP, y_target):
        return 0.5 * np.sum((y_MLP - y_target) ** 2)
    ...
    def pre_algo_sgd_momentum(self):
        # TODO (homework)
        self.momentum_w = [np.zeros_like(w) for w in self.weights_]
        self.momentum_w0 = [np.zeros_like(w0) for w0 in self.weights0_]
    
    def algo_sgd_momentum(self, l):
        # TODO: self.weights_[l], self.weights0_[l] to be updated (l is a layer index)
        self.momentum_w[l] = self.momentum_beta * self.momentum_w[l] + (1 - self.momentum_beta) * self.gradients[l]
        self.weights_[l] -= self.learning_rate * self.momentum_w[l]
        self.momentum_w0[l] = self.momentum_beta * self.momentum_w0[l] + (1 - self.momentum_beta) *  self.gradients0[l]
        self.weights0_[l] -= self.learning_rate * self.momentum_w0[l]

    def pre_algo_rmsprop(self):
        self.squared_grad_w = [np.zeros_like(w) for w in self.weights_]
        self.squared_grad_w0 = [np.zeros_like(w0) for w0 in self.weights0_]
    ...
    def algo_rmsprop(self, l):
        ...
        self.squared_grad_w0[l] = self.rmsprop_beta * self.squared_grad_w0[l] + (1 - self.rmsprop_beta) * (self.gradients0[l] ** 2)
        self.weights_[l] -= self.learning_rate * self.gradients[l] / (np.sqrt(self.squared_grad_w[l]) + self.rmsprop_epsilon)
        self.weights0_[l] -= self.learning_rate * self.gradients0[l] / (np.sqrt(self.squared_grad_w0[l]) + self.rmsprop_epsilon)

    def pre_algo_adam(self):
        self.m_t = [np.zeros_like(w) for w in self.weights_]
        self.v_t = [np.zeros_like(w) for w in self.weights_]
        self.m_t0 = [np.zeros_like(w0) for w0 in self.weights0_]
        self.v_t0 = [np.zeros_like(w0) for w0 in self.weights0_]

    def algo_adam(self, l):
        self.m_t[l] = self.adam_beta1 * self.m_t[l] + (1 - self.adam_beta1) * self.gradients[l]
        self.v_t[l] = self.adam_beta2 * self.v_t[l] + (1 - self.adam_beta2) * (self.gradients[l] ** 2)
        m_hat = self.m_t[l] / (1 - self.adam_beta1 ** (self.t + 1))
        v_hat = self.v_t[l] / (1 - self.adam_beta2 ** (self.t + 1))
        self.weights_[l] -= self.learning_rate * m_hat / (np.sqrt(v_hat) + self.adam_epsilon)

        self.m_t0[l] = self.adam_beta1 * self.m_t0[l] + (1 - self.adam_beta1) * self.gradients0[l]
        self.v_t0[l] = self.adam_beta2 * self.v_t0[l] + (1 - self.adam_beta2) * (self.gradients0[l] ** 2)
        m_hat0 = self.m_t0[l] / (1 - self.adam_beta1 ** (self.t + 1))
        v_hat0 = self.v_t0[l] / (1 - self.adam_beta2 ** (self.t + 1))
    ...
    def forward(self, X_b):
        ...
        self.signals[0] = X_b
        for i in range(1,len(self.structure) + 2):
            self.signals[i] = self.weights_[i] @ self.signals[i - 1].T + self.weights0_[i]
            self.signals[i] = self.signals[i].T
            if i < len(self.structure) + 1:
                self.signals[i] = self.activation_(self.signals[i])
            else:
                self.signals[i] = self.targets_activation_(self.signals[i])
        # TODO: carry out forward computations throughout layers
    ...
    def backward(self, y_b):
        ...
        self.deltas[-1] = self.loss_d_(self.signals[-1], y_b) * self.targets_activation_d_(self.signals[-1])
        for i in range(len(self.structure) + 1, 0, -1):
            self.gradients[i] = self.deltas[i].T @ self.signals[i - 1]
            self.gradients0[i] = np.sum(self.deltas[i], axis=0).reshape(-1, 1)
            self.deltas[i - 1] = (self.deltas[i] @ self.weights_[i]) * self.activation_d_(self.signals[i - 1])
        # TODO: carry out backward computations throughout layers
                            
    def predict(self, X):
        self.forward(X)        
        # y_pred = np.random.rand(X.shape[0], self.n_targets_) # TODO replace by: y_pred = self.signals[-1] (when self.forward(X) is ready)
