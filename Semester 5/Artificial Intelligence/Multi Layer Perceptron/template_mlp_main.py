...
def mlptTest(structs, algo, learn_rates):
    approx = MLPApproximator(structure=structs, activation_name="sigmoid",
                             targets_activation_name="linear", initialization_name="uniform",
                             algo_name=algo, learning_rate=learn_rates,
                             n_epochs=2000, batch_size=10, seed=53692,
...
if __name__ == '__main__':
    ...
    mlptTest([128,64,32], "rmsprop", 1e-3)


    # APPROXIMATOR (NEURAL NETWORK)

    # algorithms = ["sgd_simple", "sgd_momentum", "rmsprop", "adam"]
    # algorithms = ["sgd_momentum"]
    # learning_rates = [1e-2, 1e-3, 1e-4]
    # structures = [[128,64,32],[128,128,64,64,32,32],[64]*5+[32]*5+[16]*5+[8]*5]
    # for structs in structures:
    #     for algo in algorithms:
    #         for learn_rates in learning_rates:
    #             try:
    #                 mlptTest(structs, algo, learn_rates)
    #             except:
    #                 print("Error in algorithm: " + algo + " learn_rate: " + str(learn_rates) + " structure: " + str(structs))
    #                 pass